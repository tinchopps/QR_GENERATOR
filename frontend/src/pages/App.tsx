...
const App = (): JSX.Element => {
  ...
  // Keyboard shortcut: Ctrl+Enter to generate when auto disabled
  useEffect(() => {
  ...
  if (!debouncedConfig.data || !autoGenerate) return;
    // Skip auto generation if a manual generation just occurred with identical config
    ...
    if (lastManualRef.current && lastManualRef.current.used && lastManualRef.current.cfg === serialized) {
      // consume the flag so next change can auto-generate
      lastManualRef.current.used = false;
      return;
    }
    (async () => {
      try {
  const res = await axios.post<QrResponseShared>('/api/qr', debouncedConfig);
  setResult(res.data);
      } catch (e) {
  ...
  const handleGenerate = async () => {
    ...
    try {
  const res = await axios.post<QrResponseShared>('/api/qr', config);
  setResult(res.data);
    } catch (e) {
      setToast('Generation failed');
    } finally {
